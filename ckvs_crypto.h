...
#define BUFFER_SIZE 2 * CKVS_MAXKEYLEN + 2
 ...
 * *************************************************** */
/**
 * @brief Generates stretched_key, auth_key and c1 and stores them into the
 * memrecord.
 *
 ...
 */
int ckvs_client_encrypt_pwd(struct ckvs_memrecord *mr, const char *key,
                            const char *pwd);

/**
 * @brief Generates master key from c1 stored in mr and the given c2.
 *
 * @param mr (struct ckvs_memrecord*) the record which will hold master_key. It
 * must contains c1.
 ...
 * @param outbuf (unsigned char*) byte array of length at least
 * inbuflen+EVP_MAX_BLOCK_LENGTH
 * @param outbuflen (size_t*) actual number of bytes that was written in the
 * output
 * @return int, error code
 */
int ckvs_client_crypt_value(const struct ckvs_memrecord *mr,
                            const int do_encrypt, const unsigned char *inbuf,
                            size_t inbuflen, unsigned char *outbufptr,
                            size_t *outbuflen);
