...
typedef enum {
  ERR_CLANG_TYPE_FIX =
      -1, // this stupid value is to fix type to be int instead of unsigned on
          // some compilers (e.g. clang version 8.0)
  ERR_NONE = 0, // no error
...
#ifdef DEBUG
// dirty trick, waiting for N2023
// (www.open-std.org/jtc1/sc22/wg14/www/docs/n2023.pdf) to be implemented...
#define debug_printf_core(fmt, ...)                                            \
  do {                                                                         \
    fprintf(stderr, "DEBUG %s:%d:%s(): " fmt "%s\n", __FILE__, __LINE__,       \
...
// ----------------------------------------------------------------------
/**
 * @brief M_EXIT macro is useful to return an error code from a function with a
 * debug message. Example usage: M_EXIT(ERR_INVALID_ARGUMENT, "unable to do
 * something decent with value %lu", i);
 */
 ...
 *        Example usage:
 *            M_REQUIRE(i <= 3, ERR_INVALID_ARGUMENT, "input value (%lu) is too
 * high (> 3)", i);
 */
