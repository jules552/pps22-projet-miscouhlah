...
/**
 * ckvs_curl_WriteMemoryCallback -- lifted from
 ...
 * @param userp (void*) points to a ckvs_connection (set with the
 * CURLOPT_WRITEDATA option)
 * @return (size_t) the number of written bytes, or 0 if an error occured
 */
static size_t ckvs_curl_WriteMemoryCallback(void *contents, size_t size,
...
int ckvs_rpc_init(struct ckvs_connection *conn, const char *url) {
  ...
  curl_easy_setopt(conn->curl, CURLOPT_WRITEFUNCTION,
                   ckvs_curl_WriteMemoryCallback);
...
int ckvs_rpc(struct ckvs_connection *conn, const char *GET) {
  ...
  // url will hold the concatenation of conn->url and GET but don't forget `\0'
  // at the end
...
int ckvs_post(struct ckvs_connection *conn, const char *GET, const char *POST) {
  ...
  // put the header
  ret = curl_easy_setopt(conn->curl, CURLOPT_HTTPHEADER, headers);
  if (ret != CURLE_OK) {
    free(url);
  ...
  // check if the response is valid (empty)
  if (conn->resp_buf && (strlen(conn->resp_buf) != 0)) {
    pps_printf("%s\n", conn->resp_buf);
    free(url);
    curl_slist_free_all(headers);
    return ERR_IO;
